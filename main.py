...
class Ball:
    ...
    def update(self, player, ai_player):
        ...
        # 1. Sprawdzenie autów bocznych (wyjście poza lewą lub prawą stronę)
        if self.wx < SINGLES_XMIN or self.wx > SINGLES_XMAX:
            # Jeśli piłka wyszła z boku na połowie AI (wy < 0.5), punkt dla Gracza
            if self.wy < NET_WY:
                return 0 
            # Jeśli piłka wyszła z boku na połowie Gracza (wy > 0.5), punkt dla AI
            else:
                return 1

        # 2. Sprawdzenie linii końcowych (wyjście za plecy zawodnika)
        if self.wy < 0: 
            return 0 # Piłka minęła AI -> Punkt dla Gracza
        if self.wy > 1: 
            return 1 # Piłka minęła Gracza -> Punkt dla AI

        # 3. Odbicie od ziemi (żeby piłka nie przelatywała pod podłogą)
        if self.height <= 0:
            self.height = 0
